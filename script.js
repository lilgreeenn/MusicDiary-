const clientId = '4c56e498731941f9be11c928afdec994';
const clientSecret = 'beae2d0ef8fc4aefb1dce7012f70483c';
...
// 修改 generateCalendar 函数，添加月份切换功能
...
// 修改 getSpotifyRecommendations 函数，添加错误处理和重试机制
async function getSpotifyRecommendations() {
    try {
        // 检查是否达到请求限制
        const lastRequestTime = localStorage.getItem('lastRecommendationRequestTime');
        const now = Date.now();
        if (lastRequestTime && now - parseInt(lastRequestTime) < 30000) { // 30秒内不重复请求
            // 如果太频繁，就只返回本地推荐
            return getLocalRecommendations();
        }

        // 获取最近播放的歌曲 ID
        const recentTrackIds = playStats.recentPlays
            .map(song => song.song_url?.split('/track/')[1]?.split('?')[0])
            .filter(id => id)
            .slice(0, 5);
        ...
        // 使用 Spotify API 获取推荐
        const response = await fetch(`https://api.spotify.com/v1/recommendations?limit=10&seed_tracks=${recentTrackIds[0]}`, {
            headers: {
                'Authorization': `Bearer ${accessToken}`
            }
        });
        ...
        if (!response.ok) {
            if (response.status === 429) { // Too Many Requests
                console.log('API 请求次数达到限制，使用本地推荐');
                return getLocalRecommendations();
            }
        ...
        const data = await response.json();
        localStorage.setItem('lastRecommendationRequestTime', now.toString());
...
function getLocalRecommendations() {
    ...
    // 随机选择一些歌曲
    const availableSongs = [...musicLibrary.music];
    while (recommendations.length < 5 && availableSongs.length > 0) {
        const randomIndex = Math.floor(Math.random() * availableSongs.length);
        recommendations.push(availableSongs.splice(randomIndex, 1)[0]);
    }
...
// 修改 showRecommendations 函数
async function showRecommendations() {
    try {
        // 只获取 Spotify 推荐
        let spotifyRecommendations = [];
        try {
            spotifyRecommendations = await getSpotifyRecommendations();
        } catch (error) {
            console.log('Spotify recommendations failed to load');
            return; // 如果获取失败就不显示推荐
        }
        ...
        recommendContainer.querySelectorAll('.recommendation-item').forEach((item, index) => {
            ...
            item.addEventListener('click', (e) => {
                if (clickTimer === null) {
                    clickTimer = setTimeout(() => {
                        const songData = spotifyRecommendations[index];
                        playPreview(songData);
                        clickTimer = null;
                    }, 200);
                }
            });
    ...
    helpModal.innerHTML = `
        <div class="help-content">
            ...
            <div class="help-section">
                ...
                <ul>
                    <li>🎲 Click "Random Play" to play random music</li>
                    <li>🔍 Use search box to find songs</li>
                    <li>⏸️ Use pause/play button to control playback</li>
                </ul>
            </div>
            ...
            <div class="help-section">
                ...
                <ul>
                    ...
                    <li>👆👆 Double click to open in Spotify</li>
                    <li>❤️ Double click search results to add to favorites</li>
                    <li>🗑️ Click × button to remove from favorites</li>
                </ul>
            </div>
            ...
            <div class="help-section">
                ...
                <ul>
                    <li>📅 Use arrows to switch months</li>
                    <li>🖱️ Drag album covers to calendar cells</li>
                    <li>✏️ Click calendar cells to write mood notes</li>
                    <li>❌ Click × in top-right to delete content</li>
                </ul>
            </div>
            ...
            <div class="help-section">
                ...
                <ul>
                    <li>📷 Click bottom-right button to export calendar</li>
                    <li>📊 View play statistics and recommendations</li>
                    <li>💾 All data automatically saved locally</li>
                    <li>🎵 Music player window can be moved and resized</li>
                </ul>
            </div>
        </div>
    `;
